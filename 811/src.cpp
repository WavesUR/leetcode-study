...
public:
    vector<string> subdomainVisits(vector<string>& cpdomains) {
        ...
        for(int i = 0; i < cpdomains.size(); i++){
            ...
            int s = cpdomains[i].find(" ");
            int num = stoi(cpdomains[i].substr(0,s));

            for(int j = s; j < n; j++){
                if(cpdomains[i][j] == '.' || cpdomains[i][j] == ' '){
                    if(map.find(cpdomains[i].substr(j+1)) == map.end()){
                        map[cpdomains[i].substr(j+1)] = num;
                    }else{
                        map[cpdomains[i].substr(j+1)] += num;
                    }
                }
